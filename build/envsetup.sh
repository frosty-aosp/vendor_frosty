...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/frosty_"${target}"-ota.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.frosty.device | grep -q "$FROSTY_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.frosty.device | grep -q "$FROSTY_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.frosty.device | grep -q "$FROSTY_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
